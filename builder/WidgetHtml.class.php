...
class  WidgetHtml{
	...
	function html() {
		...
		function formUploadFile($source,$name,$label,$folder="",$image=TRUE,$width="80px") {
		...
		$html.='	<label for="textfield" class="control-label">'.$label.'</label>';
		$html.='	<div class="controls">';
		$html.='		<input type="file" name="'.$name.'[]" multiple >';
		$html.='		<span class="help-block">';
				
		foreach($img as $value){	 
			if($image===TRUE){
				$html.='<div class="img" style="float:left;" ><img style="width:80px; border:1px solid black" src="'.$folder."/".$value.'"><br/><input name="deleteFiles[]" value="'.$folder."/".$value.'" type="checkbox"> Eliminar</div>';
			}else{ 
				$html.='<div class="img" style="float:left;" >'.$value.'<br/><input name="deleteFiles[]" value="'.$folder."/".$value.'" type="checkbox"> Eliminar</div>';
			}
		}
		$html.='		</span>';
		$html.='		<input type="hidden" value="cadenadeimagenes">';
		$html.='		<input type="hidden" name="file_folder_'.$name.'" value="'.$folder.'">';
		$html.='	</div>';
		$html.='</div>';
	...
	public function actionUploadFile($files,$name,$folder){
	
		/*echo "<pre>";
		print_r($_REQUEST['deleteFiles']);
		echo "</pre>";
		exit; */
		if(isset($_REQUEST['deleteFiles'])){
			foreach($_REQUEST['deleteFiles'] as $value){
				if(file_exists($value)){
					if(unlink($value))
					$this->mensaje[]="El archivo fue borrado";
				}
			}
		}
        ...
        foreach($files as $file)
        {
            ...
            if($_FILES[$name]['tmp_name'][$i])
            {
                ...
                $trozos[$i] = explode(".", $_FILES[$name]["name"][$i]);
                ...
                if($this->checkExtension($extension[$i]) === TRUE)
                {
                   ...
                   // $_FILES['userfile']['name'][$i] = $this->checkExists($trozos[$i]);          
                    ...
                    if(move_uploaded_file($_FILES[$name]['tmp_name'][$i],$folder."/".$_FILES[$name]['name'][$i]))
                    {
                        $this->mensaje[]="subida correctamente";
                        //aqui podemos procesar info de la bd referente a este archivo
                    }
                //si la extension no es una de las permitidas
                }else{
                    $this->mensaje[]="la extension no esta permitida";
                }
            //si ese input file no ha sido cargado con un archivo
            }else{
                $this->mensaje[]="sin imagen";
            }
    ...
    */
    private function checkExtension($extension)
    {
        ...
        $extensiones = array("jpg","png","gif","pdf","jpeg");
	...
	function boton($configuracion, $atributos) {
		...
		if($atributos["tipo"]=="boton") {
			...
			if(!isset($atributos["cancelar"])&&(isset($atributos["verificarFormulario"])&&$atributos["verificarFormulario"]!="")) {
				$this->cadenaBoton.="onclick=\"if(".$atributos["verificarFormulario"]."){".$atributos["onsubmit"].$atributos["nombreFormulario"].".elements['".$atributos["id"]."'].value='true';document.forms['".$atributos["nombreFormulario"]."'].submit()}else{this.disabled=false;false}\">".$atributos["valor"]."</button>\n";
				...
				$this->cadenaBoton.="onclick=\"".$atributos["nombreFormulario"].".elements['".$atributos["id"]."'].value='true';document.forms['".$atributos["onsubmit"].$atributos["nombreFormulario"]."'].submit()\">".$atributos["valor"]."</button>\n";
				//El cuadro de Texto asociado
				$atributos["id"]=$atributos["id"];
				$atributos["tipo"]="hidden";
				$atributos["obligatorio"]=false;
